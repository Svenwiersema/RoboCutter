...
def _onderdeel_naar_dict(o: ModelOnderdeel) -> dict:
    return {
        "id": o.id,
        "naam": o.naam,
...
def _dict_naar_onderdeel(d: dict) -> ModelOnderdeel:
...
def _model_naar_rij(m: Model) -> tuple:
    return (
        ...
        json.dumps([_onderdeel_naar_dict(o) for o in m.onderdelen]),
...
def _rij_naar_model(rij: sqlite3.Row) -> Model:
    return Model(
        ...
        onderdelen=[_dict_naar_onderdeel(d) for d in json.loads(rij["onderdelen"])],
